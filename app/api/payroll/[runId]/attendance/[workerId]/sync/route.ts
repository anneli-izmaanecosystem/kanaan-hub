...
export async function POST(_req: NextRequest, { params }: Params) {
  ...
  const [[run], [worker], days, advRows, [entry]] = await Promise.all([
  ...
  // Build EntryInput from attendance rows
  ...
  for (const d of days) {
    ...
    if (worker.payStructure === 'hourly') {
      ...
      else if (d.dayType === 'public_holiday') ei.daysWorked   += 1  // PH = paid day
      else                                     ei.daysWorked   += 1

    } else if (worker.payStructure === 'floor') {
      if (d.dayType === 'saturday') ei.saturdayDays += 1
      // weekdays absorbed in floor; no deduction unless absent (unpaid)
    }
  }
