...
export type RecalcResult =
  | { ok: true; grossPay: number; netPay: number }
...
// Recalculate a worker's payrollEntry from saved attendance + advances.
// Single source of truth — called from both the attendance-sync route and the
// staff-log review route so the two paths can never drift out of sync again.
export async function recalculatePayrollEntry(rid: number, wid: number): Promise<RecalcResult> {
  ...
  if (run.status === 'finalised') return { ok: false, status: 403, error: 'Run is finalised — attendance/advances can no longer change its numbers' }
  if (!worker) return { ok: false, status: 404, error: 'Worker not found' }
  if (!entry)  return { ok: false, status: 404, error: 'No payroll entry for this worker in this run' }
  ...
  if (worker.payStructure === 'floor' && fuelDays.length > 0) {
    ...
    return { ok: true, grossPay: grossOverride, netPay }
  }
  ...
  return { ok: true, grossPay: calc.grossPay, netPay: calc.netPay }
}
